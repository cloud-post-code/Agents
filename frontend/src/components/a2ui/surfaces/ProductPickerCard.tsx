...
export interface ProductPickerCardProps {
  ...
  multiSelect?: boolean;
  onMultiSelect?: (products: ProductPickerItem[]) => void;
}
...
export function ProductPickerCard({
  ...
  const handleSingleSelect = (product: ProductPickerItem) => {
    if (selected) return;
    setSelected(product.id);
    onSelect?.(product);
  };

  const handleToggleCheck = (product: ProductPickerItem) => {
    setCheckedIds((prev) => {
  ...
  const handleConfirm = () => {
    const chosen = results.filter((p) => checkedIds.has(p.id));
    ...
    <div className="rounded-xl border border-amber-200 bg-amber-50 w-full max-w-md text-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-amber-100">
        <p className="font-semibold text-amber-900">
          {multiSelect ? "Select products" : "Which product did you mean?"}
        </p>
        {query && (
          ...
          <ul className="divide-y divide-amber-100">
            {visibleResults.map((product) => {
              if (multiSelect) {
                ...
                return (
                  <li key={product.id}>
                    <label className="w-full flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-amber-100 transition-colors">
                      {/* Checkbox */}
                      <input
                        type="checkbox"
                        checked={isChecked}
          ...
          {multiSelect && checkedIds.size > 0 && (
            <div className="px-4 py-3 border-t border-amber-100">
              <button
                ...
                className="w-full bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white font-semibold text-sm py-2 px-4 rounded-lg transition-colors"
              >
                Confirm selection ({checkedIds.size})
              </button>
            </div>
          )}
        </>
